...
def calculate_metrics(result: BacktestResult, config: AppConfig) -> dict[str, Any]:
    ...
    benchmark_initial = float(curve["benchmark_nav"].iloc[0])
    benchmark_final = float(curve["benchmark_nav"].iloc[-1])
    cagr = (final / initial) ** (1.0 / years) - 1.0 if initial > 0 else np.nan
    benchmark_cagr = (
        (benchmark_final / benchmark_initial) ** (1.0 / years) - 1.0
    ...
    # Half of gross buys plus sells is the common one-way-equivalent turnover convention.
    ...
    return {
        ...
        "benchmark_total_return": _finite(benchmark_final / benchmark_initial - 1.0),
        "benchmark_cagr": _finite(benchmark_cagr),
        "excess_cagr": _finite(cagr - benchmark_cagr),
...
def _format_metric(name: str, value: Any) -> str:
    ...
    if name in {"initial_nav", "final_nav", "total_fees"}:
...
def _performance_image(curve: pd.DataFrame) -> str:
    ...
    axes[0].plot(frame["date"], frame["benchmark"], label="Benchmark", linewidth=1.2)
...
def _html_report(
    ...
    warnings = [
        ...
        "日线回测无法还原盘口排队、集合竞价成交概率和盘中冲击成本。",
        "复权因子用于近似计入分红送转总回报；极端公司行为需逐笔复核。",
    ] + result.warnings
...
.grid{{display:grid;grid-template-columns:minmax(280px,0.8fr) minmax(480px,1.8fr);gap:18px}}
...
<body><main>
<h1>A股多因子策略回测报告</h1>
<p class="sub">{metrics['start_date']} 至 {metrics['end_date']} · 历史指数成分 · 月度调仓 · 次日开盘执行</p>
