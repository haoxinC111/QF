...
这是一个研究/回测级的 Python 项目。v1.3 在 v1.2 的 Tushare 严格账本回测之外，增加不需要 Token 的公开数据研究通道：可恢复下载新浪/东方财富历史日线，读取历史时点沪深300成分区间，并按开发期、验证期和冻结样本外期报告结果。严格通道仍显式建模历史行业/市值、分红送股、信号与成交错位、涨跌停、停牌、退市、T+1、100 股整手、历史费用、滑点和成交容量。
...
公开通道使用第三方项目根据中证指数官方公告整理的历史成分区间，并通过公开 HTTPS 行情接口下载日线和后复权因子。先安装额外依赖并取得历史成分文件：
...
运行六组预先定义的策略，并分别输出 2013–2017 开发期、2018–2021 验证期、2022–2025 冻结样本外期和全区间指标：
...
对预先声明的动量候选运行 5/10/20 bps 单边滑点压力和逐因子删除：
...
默认会生成“完整模型 + 逐因子剔除”、5/10/20 bps 滑点与 1/2 倍券商佣金组合、以及 5 年研究期后逐年向前滚动的固定参数样本外结果。法定印花税和过户费不会随佣金倍数放大。
...
v1.0/v1.1 缓存不能直接用于 v1.2。首次升级必须把 `data.refresh` 改为 `true`，生成包含 `securities.csv.gz`、`corporate_actions.csv.gz`、`industry_membership.csv.gz` 以及每日市值字段的 v3 缓存；完成后建议改回 `false`。
...
| `metrics.json` | 策略、全收益基准、风险匹配基准、换手、费用和公司行动指标 |
...
| 文件 | 内容 |
| --- | --- |
| `factor_ablation.csv` | 完整模型和逐个剔除五个因子的同口径指标 |
...
| 文件 | 内容 |
| --- | --- |
| `period_metrics.csv` | 六组预定义策略在开发、验证、冻结样本外和全区间的收益、风险、基准与换手 |
| `research_manifest.json` | 数据路径、冻结区间、策略参数和公开数据局限 |
...
| `robustness/cost_stress.csv` | 动量候选在 5/10/20 bps 单边滑点下的全区间与样本外结果 |
...
- 默认因子仍以价格与流动性为主；市值只用于风格控制，没有基本面质量、估值、盈利修正或完整协方差风险模型。
- 申万行业成员和每日市值依赖数据源历史覆盖与修订；下载后的 v3 缓存、配置和运行时间必须和结果一起归档。
...
当前包含 37 项测试，覆盖：历史费率边界、T+1 股份批次、NaN/非法 OHLC、行情/基准/行业缺口、数据源数字日期、行业区间覆盖、旧缓存拒绝、成分快照断档、历史时点行业、未来数据隔离、排名缓冲、行业/单股权重约束、信号日冻结股数、执行日 ST、涨停重试、现金与持仓恒等式、现金分红、送股、退市核销、陈旧估值、暴露报表、因子消融、成本压力、滚动样本外、公开成分区间边界、公开因子未来数据隔离和风险匹配基准。
