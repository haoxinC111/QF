...
这是一个研究/回测级的 Python 项目。v1.4.2 在 v1.4.1 结果审计热修复之上，把新浪 HTTP 下载与 JavaScript 解码分离：多个 worker 并发请求，但只有一个专用线程创建、调用和释放 MiniRacer。严格通道仍显式建模历史行业/市值、分红送股、信号与成交错位、涨跌停、停牌、退市、T+1、100 股整手、历史费用、滑点和成交容量。
...
运行六组预先定义的策略，并分别输出 2013–2017 开发期、2018–2021 验证期、2022–2025 已查看历史保留期和全区间指标。2022–2025 已经被用于讨论结果，后续不得再称为未触碰样本外；真正的前瞻检验应从 2026 年以后数据或模拟盘开始：
...
| 文件 | 内容 |
| --- | --- |
| `period_metrics.csv` | 六组预定义策略在开发、验证、已查看历史保留期和全区间的收益、风险、基准与换手 |
...
当前包含 46 项测试，覆盖：历史费率边界、T+1 股份批次、NaN/非法 OHLC、行情/业绩基准/择时指数/行业缺口、择时与业绩基准隔离、数据源数字日期、行业区间覆盖、旧缓存拒绝、成分快照断档、历史时点行业、未来数据隔离、排名缓冲、行业/单股权重约束、信号日冻结股数、执行日 ST、涨停重试、现金与持仓恒等式、确定性黄金结果、现金分红、送股、退市核销、陈旧估值、暴露报表、因子消融、成本压力、滚动样本外、公开成分区间边界、公开因子未来数据隔离、停牌锁仓、实验登记幂等性和严格/公开缓存篡改检测。
