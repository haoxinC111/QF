...
这是一个研究/回测级的 Python 项目。v1.2 在真实股份批次和公司行动账本之上，新增历史时点申万行业、每日市值、行业/市值中性化、行业权重上限、实际持仓排名缓冲，以及可重复的因子消融、成本压力和滚动样本外评估。历史成分、复权因子、分红送股、信号与成交错位、涨跌停、停牌、退市、T+1、100 股整手、历史费用、滑点和成交容量仍然显式建模。
...
当前包含 32 项测试，覆盖：历史费率边界、T+1 股份批次、NaN/非法 OHLC、行情/基准/行业缺口、数据源数字日期、行业区间覆盖、旧缓存拒绝、成分快照断档、历史时点行业、未来数据隔离、排名缓冲、行业/单股权重约束、信号日冻结股数、执行日 ST、涨停重试、现金与持仓恒等式、现金分红、送股、退市核销、陈旧估值、暴露报表、因子消融、成本压力、滚动样本外和风险匹配基准。
